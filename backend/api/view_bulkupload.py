"""Complete bulk-upload implementation (migrated from `views.py`).

This module contains `BulkService` and a full `BulkUploadView` compatible with
the original `views.py` behavior. The implementation mirrors the original
logic so existing routes and clients keep working while code is modularized.
"""
...
# -------- Bulk Upload & Data Analysis (migrated block) --------
