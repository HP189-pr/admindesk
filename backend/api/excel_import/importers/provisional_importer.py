# backend/api/excel_import/importers/provisional_importer.py
"""ProvisionalRecord importer shared by admin and bulk upload flows."""
...
def process_row(row, context: ImportContext) -> RowImportResult:
    ...
    prv_number = normalize_identifier(scalar(row, "prv_number"))
    if not prv_number:
        return RowImportResult(status="skipped", message="Missing prv_number", ref=prv_number)
    ...
    if doc_rec is None:
        return RowImportResult(status="skipped", message="doc_rec_id not found", ref=prv_number)

    prv_status = normalize_provisional_status(scalar(row, "prv_status"))
    if prv_status is None:
        return RowImportResult(status="skipped", message=f"Invalid prv_status: {scalar(row, 'prv_status')}", ref=prv_number)
    is_cancel = prv_status == ProvisionalStatus.CANCELLED

    prv_date = parse_excel_date(scalar(row, "prv_date")) if "prv_date" in scope else None
    if "prv_date" in scope and prv_date is None:
        return RowImportResult(status="skipped", message="Missing prv_date", ref=prv_number)
    ...
    if not is_cancel and "enrollment_no" in scope and not enrollment_key:
        return RowImportResult(status="skipped", message="Missing enrollment_no for non-cancel record", ref=prv_number)
    if not is_cancel and "enrollment_no" in scope and enrollment_key and enrollment is None:
        return RowImportResult(status="skipped", message=f"Enrollment {enrollment_key} not found", ref=prv_number)
    ...
    if "student_name" in scope:
        ...
        if not student_name and not is_cancel:
            return RowImportResult(status="skipped", message="Missing student_name for non-cancel record", ref=prv_number)
    ...
    if "prv_date" in scope and prv_date is not None:
        defaults["prv_date"] = prv_date
    if "class_obtain" in scope:
        defaults["class_obtain"] = clean_cell(scalar(row, "class_obtain"))
    if "passing_year" in scope:
        defaults["passing_year"] = normalize_month_year(scalar(row, "passing_year"))
    if "prv_degree_name" in scope:
        defaults["prv_degree_name"] = clean_cell(scalar(row, "prv_degree_name"))
    if "pay_rec_no" in scope or pay_rec_no is not None:
        defaults["pay_rec_no"] = pay_rec_no

    existing = ProvisionalRecord.objects.filter(prv_number=prv_number).first()
    if existing is None and doc_rec is not None:
        existing = ProvisionalRecord.objects.filter(doc_rec=getattr(doc_rec, "doc_rec_id", None)).first()

    if existing is None:
        obj = ProvisionalRecord(prv_number=prv_number, **defaults)
        ...
        return RowImportResult(status="created", message="Created", ref=prv_number)
    ...
    existing.save()
    sync_docrec(doc_rec, doc_rec_date=doc_rec_date, pay_rec_no=pay_rec_no, doc_remark=doc_remark)
    return RowImportResult(status="updated", message="Updated", ref=prv_number)