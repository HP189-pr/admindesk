...
const InstitutionalVerification = ({ rights = DEFAULT_RIGHTS, onToggleSidebar, onToggleChatbox }) => {
  ...
  const loadList = useCallback(
    async (queryOverride = "") => {
      ...
      try {
        ...
        const data = await fetchJSON(`/inst-verification-main/?${params.toString()}`);
  ...
  const loadStudents = useCallback(
    async (docRecId) => {
      ...
      try {
        const data = await fetchJSON(`/inst-verification-student/?doc_rec=${encodeURIComponent(docRecId)}`);
  ...
  useEffect(() => {
    ...
    docRecDebounce.current = setTimeout(async () => {
      try {
        ...
        const data = await fetchJSON(`/inst-verification/suggest-doc-rec/?${params.toString()}`);
  ...
  const handleRecInstNameChange = (value) => {
    ...
    recInstDebounce.current = setTimeout(async () => {
      try {
        const res = await fetchJSON(`/inst-verification-main/search-rec-inst/?q=${encodeURIComponent(value.trim())}`);
  ...
  const deleteStudent = async (row) => {
    ...
    try {
      await fetchJSON(`/inst-verification-student/${row.id}/`, { method: "DELETE" });
